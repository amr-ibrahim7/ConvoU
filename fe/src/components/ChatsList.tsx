...
export default function ChatsList({ searchTerm }: { searchTerm: string }) {
  ...
  if (filteredChats.length === 0) {
    return <div className="text-center text-white/50 mt-10">No chats found for `${searchTerm}`</div>;
  }
        ...
        return (
          <button
            ...
            className={`w-full p-3 rounded-xl hover:bg-white/5 transition-all group ${
              selectedUser?.id === chat.otherParticipant.id ? "bg-white/10" : ""
            }`}
          >
            <div className="flex items-center gap-3">
              <div className="relative">
                <Avatar className="h-11 w-11 border border-white/10">
                  <AvatarImage
                    src={chat.otherParticipant.profilePic || "/home/avatar.png"}
                  />
